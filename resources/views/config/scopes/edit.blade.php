...
<div class="bg-white rounded-lg border border-gray-200 p-6 max-w-lg"
    ...
    <form method="POST" action="{{ route('config.scopes.update', $scope) }}" class="space-y-4">
        ...
        <div class="rounded-md border border-amber-200 bg-amber-50/40 px-3 py-3">
            <label class="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" name="requires_signed_consent" value="1"
                       {{ old('requires_signed_consent', $scope->requires_signed_consent) ? 'checked' : '' }}
        ...
        <div class="rounded-md border border-emerald-200 bg-emerald-50/40 px-3 py-3">
            ...
            @php $selectedDocs = old('required_documents', $scope->required_documents ?? []); @endphp
            <div class="grid grid-cols-2 gap-2">
                ...
                <label class="flex items-center gap-2 cursor-pointer text-sm text-gray-800">
                    <input type="checkbox" name="required_documents[]" value="{{ $key }}"
                           {{ in_array($key, $selectedDocs, true) ? 'checked' : '' }}
