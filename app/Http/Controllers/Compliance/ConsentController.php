...
TXT;
    ...
    {
        // Admin-captured evidence lives on this app's disk; client-captured
        // evidence (digital consent taken during request submission) lives on
        // the client portal's disk, reachable via the client_local mount.
        $disk = null;
        if ($consent->evidence_file_path) {
            foreach (['local', 'client_local'] as $candidate) {
                if (Storage::disk($candidate)->exists($consent->evidence_file_path)) {
                    $disk = $candidate;
                    break;
                }
            }
